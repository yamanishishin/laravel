...
@section('header')
        ...
        <div class = "container">
	        <div class="wrapper">
		        <form action="{{ route('password.email') }}" method="POST" name="pw_reset" class="form-signin">   
                @csrf    
		            <h3 class="form-signin-heading">Welcome ! </h3>
			        <hr class="colorgraph"><br>
                    <div>
                        <label>メールアドレス</label>
			            <input type="email" id="email" class="form-control" name="email" value="{{ old('email') }}" required="" autofocus="" /> <br>
